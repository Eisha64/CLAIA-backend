...
async function lookupMedlinePlus(term) {
  ...
  try {
    ...
    const strip = (s) => (s ? s.replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&amp;/g, "&").replace(/<[^>]+>/g, "").replace(/\s+/g, " ").trim() : "");
...
async function searchRxNorm(term, limit = 6) {
  ...
  try {
    ...
    const res = await fetch(url);
    console.log(`[RxNorm] GET ${url} -> status ${res.status}`);
    if (!res.ok) return [];
    ...
    console.log(`[RxNorm] "${term}" -> ${results.length} match(es): ${results.slice(0, 3).join(" | ")}`);
    return results;
  } catch (e) {
